...
class TariffCalculator:
    ...
    def _next_schedule_change(self, now: datetime) -> tuple[datetime, TariffPeriod]:
        ...
        probe = now
        # Scan forward up to SCHEDULE_SCAN_DAYS day-boundaries to find the next moment
        # where the tariff period actually changes (skipping boundaries where the same
        # period continues, e.g. Saturday→Sunday both all-llano on a DOUBLE contract).
